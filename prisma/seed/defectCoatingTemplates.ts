...
import { getModuleDefectCoatingDefaults } from "../../src/shared/constants/coreLoggingOptionDefaults"
...
export async function seedDefectCoatingTemplates(prisma: PrismaClient) {
  for (const moduleSlug of MODULE_SLUGS) {
    ...
    for (const [index, option] of defaults.entries()) {
      await prisma.defectCoatingTemplate.upsert({
        where: {
    ...
    console.log(
      `  Seeded ${defaults.length} defect coating templates for module "${moduleSlug}"`
    )
  }
}
