...
import { getModuleWellTypeDefaults } from "../../src/shared/constants/wellLogsOptionDefaults"
...
export async function seedWellTypeTemplates(prisma: PrismaClient) {
  for (const moduleSlug of MODULE_SLUGS) {
    ...
    for (const [index, option] of defaults.entries()) {
      await prisma.wellTypeTemplate.upsert({
        where: {
    ...
    console.log(
      `  Seeded ${defaults.length} well types templates for module "${moduleSlug}"`
    )
  }
}
