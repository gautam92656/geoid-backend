...
import { getModuleDefectOpennessDefaults } from "../../src/shared/constants/coreLoggingOptionDefaults"
...
export async function seedDefectOpennessTemplates(prisma: PrismaClient) {
  for (const moduleSlug of MODULE_SLUGS) {
    ...
    for (const [index, option] of defaults.entries()) {
      await prisma.defectOpennessTemplate.upsert({
        where: {
    ...
    console.log(
      `  Seeded ${defaults.length} defect openness templates for module "${moduleSlug}"`
    )
  }
}
