...
import { getModuleCoreDefectTypeDefaults } from "../../src/shared/constants/coreLoggingOptionDefaults"
...
export async function seedCoreDefectTypeTemplates(prisma: PrismaClient) {
  for (const moduleSlug of MODULE_SLUGS) {
    ...
    for (const [index, option] of defaults.entries()) {
      await prisma.coreDefectTypeTemplate.upsert({
        where: {
    ...
    console.log(
      `  Seeded ${defaults.length} core defect type templates for module "${moduleSlug}"`
    )
  }
}
