...
import * as repo from "./log-well-log.repository"
...
function toDTO(item: NonNullable<Awaited<ReturnType<typeof repo.findByIdForUser>>>) {
  return {
    ...
    wellTypeName: item.wellTypeName,
...
export async function list(filters: repo.LogWellLogListFilters) {
...
export async function getOne(
  ...
  if (!item || item.deletedAt) throw new NotFoundError("Well testing result not found")
  return toDTO(item)
}

export type CreateLogWellLogBody = {
  wellId?: string
  depthFrom: string
  depthTo: string
  wellTypeId: string
...
export async function create(
  ...
  const item = await repo.create({
    ...
    wellTypeName: input.wellTypeName,
...
export async function update(
  ...
  logId: number,
  id: number,
  input: repo.UpdateLogWellLogInput
) {
  await assertLogForUser(userId, projectId, logId)
  const existing = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!existing || existing.deletedAt) {
    throw new NotFoundError("Well testing result not found")
  }
...
export async function remove(
  ...
  return { message: "Well testing result removed" }
}

export async function restoreWellLog(
  userId: number,
  projectId: number,
  logId: number,
  id: number
  ...
  const existing = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!existing || !existing.deletedAt) {
    throw new NotFoundError("Deleted well testing result not found")
  }

  await repo.restore(id, userId, projectId, logId)
  const restored = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!restored) throw new NotFoundError("Well testing result not found")
...
export async function copy(
  ...
  const item = await repo.create({
    ...
    wellTypeId: existing.wellTypeId,
    wellTypeName: existing.wellTypeName,
