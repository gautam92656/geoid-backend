...
export const DRILLING_OBSERVATION_GRAPHICS_DIR = "drilling-symbols"
...
function graphicDir(): string {
  return path.join(publicRoot(), DRILLING_OBSERVATION_GRAPHICS_DIR)
}

function labelFromFilename(filename: string): string {
  const base = filename.replace(/\.(jpe?g|png|svg)$/i, "")
  const match = base.match(/^symbol0*(\d+)$/i)
  if (match) return `Symbol ${String(match[1]).padStart(2, "0")}`
  return base
}

function fileUrl(filename: string): string {
  return `/api/v1/drilling-observation-graphics/files/${encodeURIComponent(filename)}`
}
...
export async function listDrillingObservationGraphics(): Promise<
