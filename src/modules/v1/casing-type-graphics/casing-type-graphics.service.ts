...
export const CASING_TYPE_GRAPHICS_DIR = "casing-type-graphics"
...
function graphicDir(): string {
  return path.join(publicRoot(), CASING_TYPE_GRAPHICS_DIR)
}

function labelFromFilename(filename: string): string {
  const base = filename.replace(/\.(jpe?g|png|svg)$/i, "")
  const match = base.match(/^graphic0*(\d+)$/i)
  if (match) return `Graphic${String(match[1]).padStart(2, "0")}`
  return base
}

function fileUrl(filename: string): string {
  return `/api/v1/casing-type-graphics/files/${encodeURIComponent(filename)}`
}
...
export async function listCasingTypeGraphics(): Promise<CasingTypeGraphicCatalogEntry[]> {
