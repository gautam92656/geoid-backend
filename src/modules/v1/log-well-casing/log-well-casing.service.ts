...
import * as repo from "./log-well-casing.repository"
...
export async function list(filters: repo.LogWellCasingListFilters) {
...
export async function getOne(
  ...
  if (!item || item.deletedAt) throw new NotFoundError("Well casing not found")
  return toDTO(item)
}

export type CreateLogWellCasingBody = {
  depthFrom?: string
  depthTo?: string
  casingTypeId: string
...
export async function update(
  ...
  logId: number,
  id: number,
  input: repo.UpdateLogWellCasingInput
) {
  await assertLogForUser(userId, projectId, logId)
  const existing = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!existing || existing.deletedAt) {
    throw new NotFoundError("Well casing not found")
  }
...
export async function remove(
  ...
  return { message: "Well casing removed" }
}

export async function restoreWellCasing(
  userId: number,
  projectId: number,
  logId: number,
  id: number
