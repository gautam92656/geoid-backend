...
export const WELL_CASING_GRAPHICS_DIR = "well-casing"
...
function graphicDir(): string {
  return path.join(publicRoot(), WELL_CASING_GRAPHICS_DIR)
}

function labelFromFilename(filename: string): string {
  const base = filename.replace(/\.(jpe?g|png|svg)$/i, "")
  const match = base.match(/^graphic_0*(\d+)$/i)
  if (match) return `CasingGraphic${String(match[1]).padStart(2, "0")}`
  return base
}

function fileUrl(filename: string): string {
  return `/api/v1/well-casing-graphics/files/${encodeURIComponent(filename)}`
}
...
export async function listWellCasingGraphics(): Promise<WellCasingGraphicCatalogEntry[]> {
