...
export const createLogWaterObservationSchema = Joi.object({
  depth: optionalTrimmed(DEPTH_MAX_LENGTH),
  observationTypeId: Joi.string().trim().min(1).max(TYPE_ID_MAX_LENGTH).required(),
  observationTypeName: Joi.string().trim().min(1).max(TYPE_NAME_MAX_LENGTH).required(),
...
export const updateLogWaterObservationSchema = Joi.object({
  depth: optionalTrimmed(DEPTH_MAX_LENGTH),
...
export const listLogWaterObservationsQuerySchema = Joi.object({
