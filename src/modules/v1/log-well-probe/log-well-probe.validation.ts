...
export const createLogWellProbeSchema = Joi.object({
  wellId: optionalTrimmed(WELL_ID_MAX_LENGTH),
  wellIdLabel: optionalTrimmed(WELL_ID_LABEL_MAX_LENGTH),
  probeTypeId: Joi.string().trim().min(1).max(TYPE_ID_MAX_LENGTH).required(),
  probeTypeName: Joi.string().trim().min(1).max(TYPE_NAME_MAX_LENGTH).required(),
...
export const updateLogWellProbeSchema = Joi.object({
  wellId: optionalTrimmed(WELL_ID_MAX_LENGTH),
  wellIdLabel: optionalTrimmed(WELL_ID_LABEL_MAX_LENGTH),
  probeTypeId: Joi.string().trim().max(TYPE_ID_MAX_LENGTH).optional(),
  probeTypeName: Joi.string().trim().max(TYPE_NAME_MAX_LENGTH).optional(),
...
export const listLogWellProbesQuerySchema = Joi.object({
