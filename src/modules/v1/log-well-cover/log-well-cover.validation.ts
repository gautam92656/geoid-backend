...
const WELL_ID_LABEL_MAX_LENGTH = 200
...
export const createLogWellCoverSchema = Joi.object({
  wellId: optionalTrimmed(WELL_ID_MAX_LENGTH),
  wellIdLabel: optionalTrimmed(WELL_ID_LABEL_MAX_LENGTH),
  wellCoverTypeId: Joi.string().trim().min(1).max(TYPE_ID_MAX_LENGTH).required(),
  wellCoverTypeName: Joi.string().trim().min(1).max(TYPE_NAME_MAX_LENGTH).required(),
  depth: optionalTrimmed(DEPTH_MAX_LENGTH),
  comments: optionalTrimmed(COMMENTS_MAX_LENGTH),
  sortOrder: Joi.number().integer().min(0).optional(),
})

export const updateLogWellCoverSchema = Joi.object({
  wellId: optionalTrimmed(WELL_ID_MAX_LENGTH),
  wellIdLabel: optionalTrimmed(WELL_ID_LABEL_MAX_LENGTH),
  wellCoverTypeId: Joi.string().trim().max(TYPE_ID_MAX_LENGTH).optional(),
  wellCoverTypeName: Joi.string().trim().max(TYPE_NAME_MAX_LENGTH).optional(),
  depth: optionalTrimmed(DEPTH_MAX_LENGTH),
  comments: optionalTrimmed(COMMENTS_MAX_LENGTH),
  sortOrder: Joi.number().integer().min(0).optional(),
}).min(1)

export const listLogWellCoversQuerySchema = Joi.object({
