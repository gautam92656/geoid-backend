...
import * as repo from "./log-well-cover.repository"
...
function toDTO(item: NonNullable<Awaited<ReturnType<typeof repo.findByIdForUser>>>) {
  return {
    ...
    wellCoverTypeId: item.wellCoverTypeId,
    wellCoverTypeName: item.wellCoverTypeName,
    depth: item.depth,
...
export async function list(filters: repo.LogWellCoverListFilters) {
...
export async function getOne(
  ...
  if (!item || item.deletedAt) throw new NotFoundError("Well cover not found")
  return toDTO(item)
}

export type CreateLogWellCoverBody = {
  wellId?: string
  wellIdLabel?: string
...
export async function create(
  ...
  const item = await repo.create({
    ...
    wellCoverTypeId: input.wellCoverTypeId,
    wellCoverTypeName: input.wellCoverTypeName,
    depth: input.depth,
...
export async function update(
  ...
  logId: number,
  id: number,
  input: repo.UpdateLogWellCoverInput
) {
  await assertLogForUser(userId, projectId, logId)
  const existing = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!existing || existing.deletedAt) {
    throw new NotFoundError("Well cover not found")
  }
...
export async function remove(
  ...
  return { message: "Well cover removed" }
}

export async function restoreWellCover(
  userId: number,
  projectId: number,
  logId: number,
  id: number
...
export async function copy(
  ...
  const item = await repo.create({
    ...
    wellCoverTypeId: existing.wellCoverTypeId,
    wellCoverTypeName: existing.wellCoverTypeName,
