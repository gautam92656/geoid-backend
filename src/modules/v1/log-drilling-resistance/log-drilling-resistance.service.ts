...
import * as repo from "./log-drilling-resistance.repository"
...
function toDTO(item: NonNullable<Awaited<ReturnType<typeof repo.findByIdForUser>>>) {
  return {
    ...
    resistanceTypeId: item.resistanceTypeId,
    resistanceTypeName: item.resistanceTypeName,
...
export async function list(filters: repo.LogDrillingResistanceListFilters) {
...
export async function getOne(
  ...
  if (!item || item.deletedAt) throw new NotFoundError("Drilling resistance not found")
  return toDTO(item)
}

export type CreateLogDrillingResistanceBody = {
  depthFrom: string
  depthTo: string
...
export async function create(
  ...
  projectId: number,
  logId: number,
  input: CreateLogDrillingResistanceBody
) {
  await assertLogForUser(userId, projectId, logId)
  ...
  const item = await repo.create({
    ...
    resistanceTypeId: input.resistanceTypeId,
    resistanceTypeName: input.resistanceTypeName,
...
export async function update(
  ...
  logId: number,
  id: number,
  input: repo.UpdateLogDrillingResistanceInput
) {
  await assertLogForUser(userId, projectId, logId)
  const existing = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!existing || existing.deletedAt) {
    throw new NotFoundError("Drilling resistance not found")
  }
...
export async function remove(
  ...
  return { message: "Drilling resistance removed" }
}

export async function restoreResistance(
  userId: number,
  projectId: number,
  logId: number,
  id: number
  ...
  const existing = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!existing || !existing.deletedAt) {
    throw new NotFoundError("Deleted drilling resistance not found")
  }

  await repo.restore(id, userId, projectId, logId)
  const restored = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!restored) throw new NotFoundError("Drilling resistance not found")
...
export async function copy(
  ...
  const item = await repo.create({
    ...
    resistanceTypeId: existing.resistanceTypeId,
    resistanceTypeName: existing.resistanceTypeName,
