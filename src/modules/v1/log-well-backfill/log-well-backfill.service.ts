...
import * as repo from "./log-well-backfill.repository"
...
function toDTO(item: NonNullable<Awaited<ReturnType<typeof repo.findByIdForUser>>>) {
  return {
    ...
    backfillTypeId: item.backfillTypeId,
    backfillTypeName: item.backfillTypeName,
...
export async function list(filters: repo.LogWellBackfillListFilters) {
...
export async function getOne(
  ...
  if (!item || item.deletedAt) throw new NotFoundError("Well backfill not found")
  return toDTO(item)
}

export type CreateLogWellBackfillBody = {
  depthFrom?: string
  depthTo?: string
...
export async function create(
  ...
  projectId: number,
  logId: number,
  input: CreateLogWellBackfillBody
) {
  await assertLogForUser(userId, projectId, logId)
  ...
  const item = await repo.create({
    ...
    backfillTypeId: input.backfillTypeId,
    backfillTypeName: input.backfillTypeName,
...
export async function update(
  ...
  logId: number,
  id: number,
  input: repo.UpdateLogWellBackfillInput
) {
  await assertLogForUser(userId, projectId, logId)
  const existing = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!existing || existing.deletedAt) {
    throw new NotFoundError("Well backfill not found")
  }
...
export async function remove(
  ...
  return { message: "Well backfill removed" }
}

export async function restoreWellBackfill(
  userId: number,
  projectId: number,
  logId: number,
  id: number
  ...
  const existing = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!existing || !existing.deletedAt) {
    throw new NotFoundError("Deleted well backfill not found")
  }

  await repo.restore(id, userId, projectId, logId)
  const restored = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!restored) throw new NotFoundError("Well backfill not found")
...
export async function copy(
  ...
  const item = await repo.create({
    ...
    backfillTypeId: existing.backfillTypeId,
    backfillTypeName: existing.backfillTypeName,
