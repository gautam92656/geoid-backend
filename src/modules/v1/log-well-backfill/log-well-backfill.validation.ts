...
export const createLogWellBackfillSchema = Joi.object({
  depthFrom: optionalTrimmed(DEPTH_MAX_LENGTH),
  depthTo: optionalTrimmed(DEPTH_MAX_LENGTH),
  backfillTypeId: Joi.string().trim().min(1).max(TYPE_ID_MAX_LENGTH).required(),
  backfillTypeName: Joi.string().trim().min(1).max(TYPE_NAME_MAX_LENGTH).required(),
  comments: optionalTrimmed(COMMENTS_MAX_LENGTH),
  sortOrder: Joi.number().integer().min(0).optional(),
})

export const updateLogWellBackfillSchema = Joi.object({
  depthFrom: optionalTrimmed(DEPTH_MAX_LENGTH),
  depthTo: optionalTrimmed(DEPTH_MAX_LENGTH),
  backfillTypeId: Joi.string().trim().max(TYPE_ID_MAX_LENGTH).optional(),
  backfillTypeName: Joi.string().trim().max(TYPE_NAME_MAX_LENGTH).optional(),
  comments: optionalTrimmed(COMMENTS_MAX_LENGTH),
  sortOrder: Joi.number().integer().min(0).optional(),
}).min(1)

export const listLogWellBackfillsQuerySchema = Joi.object({
