...
import * as repo from "./log-drilling-observation.repository"
...
function toDTO(item: NonNullable<Awaited<ReturnType<typeof repo.findByIdForUser>>>) {
  return {
    ...
    depth: item.depth,
    depthOfCasing: item.depthOfCasing,
    depthToWater: item.depthToWater,
...
export async function list(filters: repo.LogDrillingObservationListFilters) {
...
export async function getOne(
  ...
  if (!item || item.deletedAt) throw new NotFoundError("Drilling observation not found")
  return toDTO(item)
}

export type CreateLogDrillingObservationBody = {
  depth?: string
...
export async function create(
  ...
  projectId: number,
  logId: number,
  input: CreateLogDrillingObservationBody
) {
  await assertLogForUser(userId, projectId, logId)
  ...
  const item = await repo.create({
    ...
    logId,
    depth: input.depth,
    depthOfCasing: input.depthOfCasing,
    depthToWater: input.depthToWater,
...
export async function update(
  ...
  logId: number,
  id: number,
  input: repo.UpdateLogDrillingObservationInput
) {
  await assertLogForUser(userId, projectId, logId)
  const existing = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!existing || existing.deletedAt) {
    throw new NotFoundError("Drilling observation not found")
  }
...
export async function remove(
  ...
  return { message: "Drilling observation removed" }
}
...
export async function restoreObservation(
  ...
  const existing = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!existing || !existing.deletedAt) {
    throw new NotFoundError("Deleted drilling observation not found")
  }

  await repo.restore(id, userId, projectId, logId)
  const restored = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!restored) throw new NotFoundError("Drilling observation not found")
...
export async function copy(
  ...
  const item = await repo.create({
    ...
    depthOfCasing: existing.depthOfCasing,
    depthToWater: existing.depthToWater,
