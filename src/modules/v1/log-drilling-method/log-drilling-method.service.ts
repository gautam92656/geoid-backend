...
import * as repo from "./log-drilling-method.repository"
...
function toDTO(item: NonNullable<Awaited<ReturnType<typeof repo.findByIdForUser>>>) {
  return {
    ...
    drillingMethodId: item.drillingMethodId,
    drillingMethodName: item.drillingMethodName,
    windowedWindowless: item.windowedWindowless,
    diameter: item.diameter,
    recovery: item.recovery,
    waterAdded: item.waterAdded,
...
export async function list(filters: repo.LogDrillingMethodListFilters) {
...
export async function getOne(
  ...
  if (!item || item.deletedAt) throw new NotFoundError("Drilling method not found")
  return toDTO(item)
}

export type CreateLogDrillingMethodBody = {
  depthFrom: string
  depthTo: string
...
export async function create(
  ...
  projectId: number,
  logId: number,
  input: CreateLogDrillingMethodBody
) {
  await assertLogForUser(userId, projectId, logId)
  ...
  const item = await repo.create({
    ...
    drillingMethodId: input.drillingMethodId,
    drillingMethodName: input.drillingMethodName,
    windowedWindowless: input.windowedWindowless,
...
export async function update(
  ...
  logId: number,
  id: number,
  input: repo.UpdateLogDrillingMethodInput
) {
  await assertLogForUser(userId, projectId, logId)
  const existing = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!existing || existing.deletedAt) {
    throw new NotFoundError("Drilling method not found")
  }
...
export async function remove(
  ...
  return { message: "Drilling method removed" }
}

export async function restoreMethod(
  userId: number,
  projectId: number,
  logId: number,
  id: number
  ...
  const existing = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!existing || !existing.deletedAt) {
    throw new NotFoundError("Deleted drilling method not found")
  }

  await repo.restore(id, userId, projectId, logId)
  const restored = await repo.findByIdForUser(id, userId, projectId, logId)
  if (!restored) throw new NotFoundError("Drilling method not found")
...
export async function copy(
  ...
  const item = await repo.create({
    ...
    drillingMethodId: existing.drillingMethodId,
    drillingMethodName: existing.drillingMethodName,
    windowedWindowless: existing.windowedWindowless,
    diameter: existing.diameter,
    recovery: existing.recovery,
    waterAdded: existing.waterAdded,
