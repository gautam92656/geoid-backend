import Joi from "joi"
import { DEFAULT_LIMIT, DEFAULT_PAGE, MAX_LIMIT } from "../../../shared/constants"

const DEPTH_MAX_LENGTH = 50
const METHOD_ID_MAX_LENGTH = 100
const METHOD_NAME_MAX_LENGTH = 200
const WINDOWED_MAX_LENGTH = 20
const FIELD_MAX_LENGTH = 50
const COMMENTS_MAX_LENGTH = 5000

const optionalTrimmed = (max: number) => Joi.string().trim().max(max).allow("").optional()

export const createLogDrillingMethodSchema = Joi.object({
  depthFrom: Joi.string().trim().min(1).max(DEPTH_MAX_LENGTH).required(),
  depthTo: Joi.string().trim().min(1).max(DEPTH_MAX_LENGTH).required(),
  drillingMethodId: Joi.string().trim().min(1).max(METHOD_ID_MAX_LENGTH).required(),
  drillingMethodName: Joi.string().trim().min(1).max(METHOD_NAME_MAX_LENGTH).required(),
  windowedWindowless: Joi.string()
    .trim()
    .max(WINDOWED_MAX_LENGTH)
    .valid("windowed", "windowless", "")
    .allow("")
    .optional(),
  diameter: optionalTrimmed(FIELD_MAX_LENGTH),
  recovery: optionalTrimmed(FIELD_MAX_LENGTH),
  waterAdded: optionalTrimmed(FIELD_MAX_LENGTH),
  comments: optionalTrimmed(COMMENTS_MAX_LENGTH),
  sortOrder: Joi.number().integer().min(0).optional(),
})

export const updateLogDrillingMethodSchema = Joi.object({
  depthFrom: optionalTrimmed(DEPTH_MAX_LENGTH),
  depthTo: optionalTrimmed(DEPTH_MAX_LENGTH),
  drillingMethodId: Joi.string().trim().max(METHOD_ID_MAX_LENGTH).optional(),
  drillingMethodName: Joi.string().trim().max(METHOD_NAME_MAX_LENGTH).optional(),
  windowedWindowless: Joi.string()
    .trim()
    .max(WINDOWED_MAX_LENGTH)
    .valid("windowed", "windowless", "")
    .allow("")
    .optional(),
  diameter: optionalTrimmed(FIELD_MAX_LENGTH),
  recovery: optionalTrimmed(FIELD_MAX_LENGTH),
  waterAdded: optionalTrimmed(FIELD_MAX_LENGTH),
  comments: optionalTrimmed(COMMENTS_MAX_LENGTH),
  sortOrder: Joi.number().integer().min(0).optional(),
}).min(1)

export const listLogDrillingMethodsQuerySchema = Joi.object({
  page: Joi.number().integer().min(DEFAULT_PAGE).default(DEFAULT_PAGE),
  limit: Joi.number().integer().min(1).max(MAX_LIMIT).default(DEFAULT_LIMIT),
  includeDeleted: Joi.string().valid("true", "false").optional(),
  onlyDeleted: Joi.string().valid("true", "false").optional(),
  search: Joi.string().trim().optional(),
  sortBy: Joi.string()
    .valid(
      "id",
      "depthFrom",
      "depthTo",
      "drillingMethodName",
      "windowedWindowless",
      "diameter",
      "recovery",
      "waterAdded",
      "comments",
      "sortOrder",
      "createdAt",
      "updatedAt"
    )
    .optional(),
  sortOrder: Joi.string().valid("asc", "desc").optional(),
})

export const projectLogParamsSchema = Joi.object({
  projectId: Joi.number().integer().min(1).required(),
  logId: Joi.number().integer().min(1).required(),
})
