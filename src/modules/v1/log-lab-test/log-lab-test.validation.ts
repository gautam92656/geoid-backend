...
const optionalSampleId = Joi.alternatives()
  .try(Joi.number().integer().min(1), Joi.string().trim().pattern(/^\d+$/))
  .optional()
  .allow(null, "")

export const createLogLabTestSchema = Joi.object({
  ...
  sampleId: optionalSampleId,
  sampleNo: optionalTrimmed(SAMPLE_NO_MAX_LENGTH),
  sortOrder: Joi.number().integer().min(0).optional(),
})

export const updateLogLabTestSchema = Joi.object({
...
export const listLogLabTestsQuerySchema = Joi.object({
...
export const listLogLabTestTypeGroupsQuerySchema = Joi.object({
  includeDeleted: Joi.string().valid("true", "false").optional(),
  onlyDeleted: Joi.string().valid("true", "false").optional(),
})
