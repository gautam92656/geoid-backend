...
export const WELL_BACKFILL_GRAPHICS_DIR = "well-backfill"
...
function graphicDir(): string {
  return path.join(publicRoot(), WELL_BACKFILL_GRAPHICS_DIR)
}

function labelFromFilename(filename: string): string {
  const base = filename.replace(/\.(jpe?g|png|svg)$/i, "")
  const match = base.match(/^0*(\d+)$/)
  if (match) return `BackfillGraphic${String(match[1]).padStart(2, "0")}`
  return base
}

function fileUrl(filename: string): string {
  return `/api/v1/well-backfill-graphics/files/${encodeURIComponent(filename)}`
}
...
export async function listWellBackfillGraphics(): Promise<WellBackfillGraphicCatalogEntry[]> {
